...
def selection_pair(popilation: Population, fitness_function: FitnessFunc) -> Population:
    return choices(
        population=popilation,
        weights=[fitness_function(genome) for genome in popilation],
        k=2
    )
...
def single_point_crossover(a: Genome, b: Genome) -> Tuple[Genome, Genome]:
    if len(a) != len(b):
        raise ValueError("a and b must be same of length")
...
print(f"number of genrations: {generations}")
